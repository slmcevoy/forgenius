...
# # remove positions covered in less than half of the total samples
...
bcftools view "$OUTDIR"/SNPs_HDfilt.vcf.gz \
 ...
 | bcftools view -i "(N_PASS(FORMAT/DP>=6)>=$half_samples)" \
...
subset_snps() {
	...
	# # remove possible samples with >80% missing data and remove positions that are no longer polymorphic
	 ...
	 | bcftools view -i "(N_PASS(FORMAT/GT=='RA') + N_PASS(FORMAT/GT=='AA')>=1)" \
	 | bcftools view -e '(COUNT(GT="AA")+COUNT(GT="mis"))=N_SAMPLES || (COUNT(GT="RR")+COUNT(GT="mis"))=N_SAMPLES' \
	 --output-type z > "$OUTDIR"/SNP_sampleFilt"$1".vcf.gz
	tabix "$OUTDIR"/SNP_sampleFilt"$1".vcf.gz
	bcftools stats "$OUTDIR"/SNP_sampleFilt"$1".vcf.gz > "$OUTDIR"/statistics/SNP_sampleFilt"$1".stats
}
...
subset_invar_concat_all() {
	...
	bcftools concat \
		--allow-overlaps \
		"$OUTDIR"/SNP_sampleFilt"$1".vcf.gz "$OUTDIR/$SPECIES"_SPET_invariant_sampleFilt"$1".vcf.gz \
		-O z -o "$OUTDIR"/"$SPECIES"_SPET_allsites_filt"$1".vcf.gz
	tabix "$OUTDIR"/"$SPECIES"_SPET_allsites_filt"$1".vcf.gz
	bcftools stats "$OUTDIR"/"$SPECIES"_SPET_allsites_filt"$1".vcf.gz > "$OUTDIR"/statistics/"$SPECIES"_SPET_allsites_filt"$1".stats
	}
