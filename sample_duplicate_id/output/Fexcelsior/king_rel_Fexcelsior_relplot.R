## king_rel_Fexcelsior_relplot.R for KING --related, by Wei-Min Chen
rm(list=ls(all=TRUE))
postscript("king_rel_Fexcelsior_relplot.ps", paper="letter", horizontal=T)
data<-c()
if(file.exists("king_rel_Fexcelsior.kin")) data <- read.table(file="king_rel_Fexcelsior.kin", header=T)
...
main = "IBD Segments In king_rel_Fexcelsior Families",
...
main = paste("Kinship vs Proportion IBD (Corr=", round(cor(data$Kinship[data$PropIBD>0], data$PropIBD[data$PropIBD>0]),digit=3),") in king_rel_Fexcelsior Families",sep=""),
...
main = "Kinship vs IBS0 in king_rel_Fexcelsior Families",
...
main = "Kinship vs Heterozygote Concordance In king_rel_Fexcelsior Families",
...
if(!file.exists("king_rel_Fexcelsior.kin0")) quit()
data <- read.table(file = "king_rel_Fexcelsior.kin0", header=T)
if(length(data$IBD1Seg)>0 & length(data$IBD2Seg)>0){
d0 <- data$IBD2Seg>0.7
...
main = "IBD Segments In Inferred king_rel_Fexcelsior Relatives",
xlab=expression(paste("Length Proportion of IBD1 Segments (", pi[1], ")",sep="")),
ylab=expression(paste("Length Proportion of IBD2 Segments (", pi[2], ")",sep="")))
points(data$IBD1Seg[d0], data$IBD2Seg[d0], col="purple")
points(data$IBD1Seg[d1.PO], data$IBD2Seg[d1.PO], col="red")
...
ylim=c(min(data$Kinship), max(data$Kinship)),
main = paste("Kinship vs Proportion IBD (Corr=", round(cor(data$Kinship[data$PropIBD>0], data$PropIBD[data$PropIBD>0]),digit=3),") in Inferred king_rel_Fexcelsior Relatives",sep=""),
xlab=expression(paste("Proportion of Genomes IBD (", pi,"=",pi[2]+pi[1]/2,")",sep="")), 
ylab = expression(paste("Estimated Kinship Coefficient (", phi, ")",sep="")))
points(data$PropIBD[d1.PO], data$Kinship[d1.PO], col="red")
points(data$PropIBD[d0], data$Kinship[d0], col="purple")
