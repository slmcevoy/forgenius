## /users/mcevoysu/scratch/output/king/Qrobur_multispecies/king_related_Qrobur_multispecies_uniqfam0plot.R for KING --related, by Wei-Min Chen and Zhennan Zhu
rm(list=ls(all=TRUE))
library(igraph)
data <- read.table(file="/users/mcevoysu/scratch/output/king/Qrobur_multispecies/king_related_Qrobur_multispecies.kin0", header=TRUE, stringsAsFactors=FALSE)[,c("ID1", "ID2", "InfType")]
...
postscript("/users/mcevoysu/scratch/output/king/Qrobur_multispecies/king_related_Qrobur_multispecies_uniqfam0plot.ps", paper="letter", horizontal=T)
plot(uniq.cluster, vertex.color=NA, vertex.size=1, vertex.label=NA, layout=lo, asp=0,
  edge.color=Inf.color[as.numeric(E(uniq.cluster)$color)], main="All Unique Family Configurations in /users/mcevoysu/scratch/output/king/Qrobur_multispecies/king_related_Qrobur_multispecies")
